...
FROM intel/oneapi-basekit:2024.1.0-ubuntu22.04
...
# ------------------------------------------------------------
# System setup and Intel repositories
# ------------------------------------------------------------
RUN set -eux; \
    apt-get update && \
...
RUN set -eux; \
    ...
    echo "🔍 Detecting Python ABI tag..."; \
    PY_TAG="$(python3 -c 'import sysconfig; print(sysconfig.get_config_var(\"py_version_nodot\"))')"; \
    ...
    if python3 -m pip install --no-cache-dir --break-system-packages \
        ...
        python3 -m pip install --no-cache-dir --break-system-packages torch==2.3.1 torchvision==0.18.1 || (echo "ERROR: torch install failed"; exit 1); \
        python3 -m pip install --no-cache-dir --break-system-packages intel-extension-for-pytorch==2.3.110+xpu || echo "⚠️ ipex +xpu wheel not found on fallback — may require Intel index"; \
    fi

# ------------------------------------------------------------
# Install OpenVINO Toolkit + OpenVINO Model Server (OVMS)
# ------------------------------------------------------------
RUN set -eux; \
    python3 -m pip install --no-cache-dir --break-system-packages openvino==2024.3.0 openvino-dev==2024.3.0; \
