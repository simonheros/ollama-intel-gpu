...
FROM intel/oneapi-basekit:2024.2.0-ubuntu22.04
...
RUN set -eux; \
    ...
    if python3 -m pip install --no-cache-dir \
        ...
        python3 -m pip install --no-cache-dir intel-extension-for-pytorch==2.3.110+xpu || echo "⚠️ ipex +xpu wheel not found on fallback — may require Intel index"; \
    fi
...
# ------------------------------------------------------------
RUN set -eux; \
    python3 -m pip install --no-cache-dir openvino==2024.3.0 openvino-dev==2024.3.0; \
